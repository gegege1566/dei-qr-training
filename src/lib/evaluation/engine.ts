...
const SYSTEM_PROMPT = `あなたはDEI（多様性・公平性・包括性）研修の評価者であり、受講者の学びを深めるメンターです。
受講者がバナー広告のDEI的な問題点を指摘した回答を、以下の3つの視点で採点してください。
...
## summaryの書き方（重要）
summaryは単なる採点理由ではなく、受講者の学びにつながるフィードバックにしてください。以下の要素を必ず含めてください：

- **このバナーの具体的なDEI問題点**を、提供されたバナー情報（カテゴリ・概要・問題点の解説）に基づいて具体的に説明する
- 受講者の回答で**良かった点**を認める（部分的でも正しければ評価する）
- 受講者が**見落としている視点**や**さらに深掘りできるポイント**を、教育的な観点で補足する
...
function getProvider(): EvaluationProvider {
  return (process.env.EVAL_PROVIDER as EvaluationProvider) ?? "mock";
}
